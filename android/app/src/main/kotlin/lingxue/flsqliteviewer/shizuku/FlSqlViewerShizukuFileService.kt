...
class FlSqlViewerShizukuFileService() : IFlSqlViewerShizukuFileService.Stub() {
    ...
    override fun readFile(path: String): ByteArray {
        ...
        for (candidate in candidatePathsForShizuku(path)) {
            ...
            lastError =
                when {
                    file.exists() -> "Target is not a file"
                    else -> "File not found or inaccessible"
                }
        }
        throw IllegalStateException(lastError ?: "File read failed")
    }
    ...
    override fun writeFile(path: String, bytes: ByteArray) {
        ...
        for (candidate in candidatePathsForShizuku(path)) {
            ...
            if (parent != null && !parent.exists() && !parent.mkdirs()) {
                lastError = "Failed to create parent directory"
                continue
            }
            val tempFile = File("${targetFile.path}.__flsql_tmp__")
            try {
                tempFile.writeBytes(bytes)
                if (targetFile.exists() && !targetFile.delete()) {
                    throw IllegalStateException("Failed to replace target file")
                }
                if (!tempFile.renameTo(targetFile)) {
                    throw IllegalStateException("Failed to move temp file into place")
                }
                return
            } catch (error: Throwable) {
                tempFile.delete()
                lastError = error.message ?: "File write failed"
            }
        }
    ...
    private fun listEntriesInternal(path: String): List<DirectoryEntry> {
        ...
        for (candidate in candidatePathsForShizuku(normalized)) {
            ...
            lastError =
                when {
                    directory.exists() && !directory.isDirectory -> "Target is not a directory"
                    else -> "Directory not found or inaccessible"
                }
        }
        if (isAndroidDataRoot(normalized)) {
        ...
        throw IllegalStateException(lastError ?: "Directory listing failed")
    }
    ...
    companion object {
        private const val TAG = "FlSqlViewerShizukuFs"
        private const val DEFAULT_PACKAGE_NAME = "lingxue.flsqliteviewer"
